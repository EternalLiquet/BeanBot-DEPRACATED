...
public class ReactionRoleService : IDisposable, IAsyncDisposable
{
        ...
        int mutationCoordinationCapacity = 256)
    {
        ...
        _reactionRoleRepository = reactionRoleRepository ?? throw new ArgumentNullException(nameof(reactionRoleRepository));
        _client = client;
        _shutdownDrainTimeout = shutdownDrainTimeout;
        _logger = logger ?? throw new ArgumentNullException(nameof(logger));
        _roleSettings = new BoundedReactionRoleSettingsCache(cacheCapacity);
    ...
    internal async Task<ReactionRoleSettings?> GetCachedRoleSettingAsync(
        ulong messageId,
        CancellationToken cancellationToken)
    {
    ...
    private async Task EnsureCacheLoadedAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var recentSettings = await _reactionRoleRepository.GetRecentRoleSettings(
