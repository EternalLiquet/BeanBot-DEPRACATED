...
namespace BeanBot.Tests.Discord.Fortunes;

public class FortuneResponseEditServiceTests
{
    [Theory]
    ...
    {
        Assert.True(FortuneResponseEditService.IsFortuneCommand(content));
    }
    ...
    {
        Assert.False(FortuneResponseEditService.IsFortuneCommand(content));
    }
    ...
    {
        Assert.True(FortuneResponseEditService.IsFortuneCommand(content, 123));
    }

    [Fact]
    public void IsFortuneCommand_RejectsAnotherUsersMentionPrefix()
    {
        Assert.False(FortuneResponseEditService.IsFortuneCommand("<@456> fortune question", 123));
    }
    ...
    public async Task ReplaceResponseAsync_Success_PreservesWarningAndUsesCancelableRequest()
    {
        ...
        await FortuneResponseEditService.ReplaceResponseAsync(
            (replacement, options) =>
            {
                content = replacement;
        ...
        Assert.Equal(FortuneResponseEditService.EditWarning, content);
    ...
    public async Task ReplaceResponseAsync_ShutdownCancellation_StopsRetryDelay()
    {
        ...
        var operation = FortuneResponseEditService.ReplaceResponseAsync(
            (_, _) =>
            {
                calls++;
    ...
    public async Task ResolvePreviousMessageAsync_StalledLookup_TimesOutAndTracksLateTask()
    {
        ...
        await Assert.ThrowsAsync<TimeoutException>(
            () => FortuneResponseEditService.ResolvePreviousMessageAsync(
                options =>
                {
                    requestToken = options.CancelToken;
                    return completion.Task;
