...
public class FortuneMessageEditHandlerTests
{
    [Fact]
    public async Task TryTrackOperation_CapacityIsBounded()
    {
        ...
        var completions = Enumerable.Range(0, FortuneMessageEditHandler.MaximumInFlightOperations)
    ...
    public async Task TryTrackOperation_LateDiscordOperationsRemainInsideCapacityBound()
    {
        using var client = new DiscordSocketClient();
        using var handler = CreateHandler(client);
        var inFlightCompletions = Enumerable.Range(0, FortuneMessageEditHandler.MaximumInFlightOperations - 1)
            .Select(_ => new TaskCompletionSource(TaskCreationOptions.RunContinuationsAsynchronously))
            .ToArray();
    ...
    private static FortuneMessageEditHandler CreateHandler(DiscordSocketClient client)
    {
        var service = new FortuneResponseEditService(
            client,
            NullLogger<FortuneResponseEditService>.Instance);
        return new FortuneMessageEditHandler(
            client,
            service,
            NullLogger<FortuneMessageEditHandler>.Instance);
    }
}
