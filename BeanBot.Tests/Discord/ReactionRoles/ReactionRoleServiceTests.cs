...
public class ReactionRoleServiceTests
{
    [Fact]
    public async Task ApplicationShutdown_RawRoleMutationSurvivesDrain_PreventsDiscordTeardown()
    {
        ...
        host.Services.GetRequiredService<ReactionRoleHandler>().InitializeReactDependentServices();
    ...
    public async Task GetCachedRoleSettingAsync_PreloadUsesCacheCapacity()
    {
        ...
        var store = new FakeReactionRoleSettingsStore
        {
            GetRecent = (_, limit, _) =>
            {
                ...
                return Task.FromResult(new List<ReactionRoleSettings> { newest, older });
            }
        };
    ...
    public async Task GetCachedRoleSettingAsync_EvictedSettingReloadsFromStore()
    {
        ...
        {
            GetByMessageId = (messageId, _) =>
                Task.FromResult<ReactionRoleSettings?>(CreateRoleSettings(messageId))
        };
        await using var service = CreateService(
            TimeSpan.FromSeconds(1),
            store,
    ...
    public async Task GetCachedRoleSettingAsync_DoesNotCacheInfrastructureFailure()
    {
        ...
        store.GetByMessageId = (_, _) => store.GetByMessageIdCallCount == 1
            ? Task.FromException<ReactionRoleSettings?>(failure)
            : Task.FromResult<ReactionRoleSettings?>(expected);
        await using var service = CreateService(TimeSpan.FromSeconds(1), store);
    ...
    public async Task GetCachedRoleSettingAsync_RetriesFailedInitialCacheLoad()
    {
        ...
        store.GetRecent = (_, _, _) => store.GetRecentCallCount == 1
            ? Task.FromException<List<ReactionRoleSettings>>(failure)
            : Task.FromResult(new List<ReactionRoleSettings> { expected });
        await using var service = CreateService(TimeSpan.FromSeconds(1), store);
    ...
    private static ReactionRoleService CreateService(
        ...
        FakeReactionRoleSettingsStore? store = null,
        int cacheCapacity = 256,
        TimeSpan? roleMutationTimeout = null,
        CancellationToken applicationStopping = default)
    {
        return new ReactionRoleService(
            new ReactionRoleRepository(
                store ?? new FakeReactionRoleSettingsStore(),
    ...
    private static SemaphoreSlim GetCacheLock(ReactionRoleService service)
    {
        var cacheLockField = typeof(ReactionRoleService).GetField(
            "_cacheLock",
